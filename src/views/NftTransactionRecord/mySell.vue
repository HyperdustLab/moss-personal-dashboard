...
const searchData = reactive({
  column: 'createTime',
  order: 'desc',
  seller: useUserStore().walletAddress || useUserStore().username,
  blockNumber: '',
  createTime_begin: '',
  createTime_end: '',
})
  ...
  <div class="app-container">
    <el-card v-loading="loading" shadow="never" class="search-wrapper">
      <el-form ref="searchFormRef" :inline="true" :model="searchData">
        ...
        <el-form-item :label="t('NftTransactionRecord.buyer')">
          <el-input v-model="searchData.buyer" :placeholder="$t('placeholderTxt')"></el-input>
        </el-form-item>
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData">
          ...
          <el-table-column width="160" :label="t('NftTransactionRecord.buyer')" align="center">
            <template #default="{ row }">
              <Substring :value="row.buyer"></Substring>
            </template>
          </el-table-column>
    ...
    <div style="height: 60px; text-align: center; margin-top: 20px">
      <el-tag style="font-size: 15px; height: 40px; padding-top: 10px">
        <span>{{ $t('NftTransactionRecord.revenueAmount') }}: {{ toAmount(sumData || 0) }}HYPT</span>
      </el-tag>
    </div>
  </div>
</template>
