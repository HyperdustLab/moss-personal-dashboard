...
<template>
  <div class="app-container">
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData">
          ...
          <el-table-column :label="t('Agent SID')" align="center">
            <template #default="{ row }">
              <Substring v-if="row.agent" :value="row.agent.sid" />
            </template>
          </el-table-column>

          <el-table-column label="Avatar" align="center">
            <template #default="{ row }">
              <el-image v-if="row.agent" style="width: 100px; height: 100px" :src="row.agent.avatar" fit="contain" />
            </template>
          </el-table-column>

          <el-table-column label="Agent Nick Name" align="center">
            <template #default="{ row }">
              <template v-if="row.agent">{{ row.agent.nickName }} </template>
            </template>
          </el-table-column>

          <el-table-column label="Agent Energy Value" align="center">
            <template #default="{ row }">
              <template v-if="row.agent">{{ row.agent.energyValue }} </template>
            </template>
          </el-table-column>

          <el-table-column label="Agent Level" align="center">
            <template #default="{ row }">
              <template v-if="row.agent">{{ row.agent.groundRodLevel }} </template>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" :label="t('actionBtnTxt')" width="150" align="center">
            <template #default="{ row }">
              <el-button @click="refPOPAgentForm.show(row)" type="primary" link>Edit</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <POPAgentForm ref="refPOPAgentForm"></POPAgentForm>
  </div>
</template>
