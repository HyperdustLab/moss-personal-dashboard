...
const searchData = reactive({
  account: useUserStore().walletAddress,
  column: 'createTime',
  order: 'desc',
})
const getTableData = async () => {
  ...
  const { result } = await api.get('/mgn/basicRewardRecord/list', searchData)
...
async function sum() {
  const { result } = await api.get('/mgn/basicRewardRecord/sum', searchData)
  ...
  <div class="app-container">
    <el-card v-loading="loading" shadow="never" class="search-wrapper">
      <el-form ref="searchFormRef" :inline="true" :model="searchData">
        <el-form-item :label="t('basicRewardRecord.nodeId')">
          <el-input v-model="searchData.nodeId" :placeholder="t('placeholderTxt')" />
        </el-form-item>
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData">
          <el-table-column prop="nodeId" :label="t('basicRewardRecord.nodeId')" align="center" />
          <el-table-column prop="amount" :label="t('basicRewardRecord.amount')" align="center">
          ...
          <el-table-column prop="blockchainId_dictText" :label="t('basicRewardRecord.blockchainNetwork')" align="center" /> >
    ...
    <div style="height: 60px; text-align: center; margin-top: 20px">
      <el-tag style="font-size: 15px; height: 40px; padding-top: 10px">
        <span>{{ $t('basicRewardRecord.sumAmount') }}: {{ toAmount(sumData || 0) }}HYPT</span>
      </el-tag>
    </div>
  </div>
</template>
